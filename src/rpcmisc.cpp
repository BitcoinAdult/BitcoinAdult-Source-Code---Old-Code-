...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    Object zBitcoinAdultObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zBitcoinAdultObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zBitcoinAdultObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zXGsupply", zBitcoinAdultObj));
...
#ifdef ENABLE_WALLET
        // Case 1: BitcoinAdult address and we have full public key:

...
#include "config/BitcoinAdult-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 8120, 8119));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, BitcoinAdult, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BitcoinAdult specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:8120"));
    ...
    strUsage += HelpMessageOpt("-backupzBitcoinAdult=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBitcoinAdult minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeBitcoinAdultamount=<n>     " + strprintf(_("Keep N BTAD anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8119, 38843));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The BitcoinAdult Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that BitcoinAdult is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BitcoinAdult Core is shutting down."));
...
#endif
    // Make sure only a single BitcoinAdult process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BitcoinAdult Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BitcoinAdult version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // BitcoinAdult: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZXGMinted();
                        RecalculateZXGSpent();
                    }
                    RecalculateXGSupply(1);
                }
                ...
                // BitcoinAdult: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BitcoinAdult Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart BitcoinAdult Core to complete") << "\n";
        ...
        bool fEnableZBitcoinAdultBackups = GetBoolArg("-backupzBitcoinAdult", true);
        pwalletMain->setZBitcoinAdultAutoBackups(fEnableZBitcoinAdultBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeBitcoinAdultAmount = 2;
...
//    }
//
//    nAnonymizeBitcoinAdultAmount = GetArg("-anonymizeBitcoinAdultamount", 0);
//    if (nAnonymizeBitcoinAdultAmount > 999999) nAnonymizeBitcoinAdultAmount = 999999;
//    if (nAnonymizeBitcoinAdultAmount < 2) nAnonymizeBitcoinAdultAmount = 2;
    ...
    LogPrintf("Anonymize BitcoinAdult Amount %d\n", nAnonymizeBitcoinAdultAmount);

...
#include "zBitcoinAdultcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBTAD ought to be enough for anybody." - Bill Gates, 2017
    ui->zXGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXGSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXG->setEnabled(false);
        ui->pushButtonMintzXG->setToolTip(tr("zBTAD is currently disabled due to maintenance."));

        ui->pushButtonSpendzXG->setEnabled(false);
        ui->pushButtonSpendzXG->setToolTip(tr("zBTAD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXGpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBTAD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBTAD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBTAD in ") +
...
void PrivacyDialog::on_pushButtonSpendzXG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBTAD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBTAD
        sendzXG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBTAD
    sendzXG();
}

void PrivacyDialog::on_pushButtonZBitcoinAdultControl_clicked()
{
    ZBitcoinAdultControlDialog* zBitcoinAdultControl = new ZBitcoinAdultControlDialog(this);
    zBitcoinAdultControl->setModel(walletModel);
    zBitcoinAdultControl->exec();
}

void PrivacyDialog::setZBitcoinAdultControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBitcoinAdultSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BitcoinAdult Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBTAD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BTAD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBTAD</b>";
    ...
    // use mints from zBitcoinAdult selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBitcoinAdultControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBitcoinAdultControlDialog::GetSelectedMints();
    }
    ...
    // Clear zBitcoinAdult selector in case it was used
    ZBitcoinAdultControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBitcoinAdult Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BitcoinAdult, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBitcoinAdult Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBTAD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBTAD "));
    ui->labelzXGAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXGSyncStatus->setVisible(fShow);
}

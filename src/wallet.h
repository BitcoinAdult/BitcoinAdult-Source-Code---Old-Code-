...
// 8119 = 1*5000 + 1*1000 + 1*500 + 1*100 + 1*50 + 1*10 + 1*5 + 1
static const int ZQ_6110 = 8119;
...
enum ZerocoinSpendStatus {
    ZXG_SPEND_OKAY = 0,                            // No error
    ZXG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXG_SPENT_USED_ZXG = 14                       // Coin has already been spend
};
...
public:
    ...
    void ZBitcoinAdultBackupWallet();
    ...
    */
    ...
    void setZBitcoinAdultAutoBackups(bool fEnabled)
    {

...
#if !defined(HAVE_MSG_XGIGNAL) && !defined(MSG_XGIGNAL)
#define MSG_XGIGNAL 0
#endif
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (BitcoinAdult users should be well used to small numbers with many decimal places by now :)
...
void SocketSendData(CNode* pnode)
{
    ...
    while (it != pnode->vSendMsg.end()) {
        ...
        int nBytes = send(pnode->hSocket, &data[pnode->nSendOffset], data.size() - pnode->nSendOffset, MSG_XGIGNAL | MSG_DONTWAIT);
        if (nBytes > 0) {
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "BitcoinAdult " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    setsockopt(hListenSocket, SOL_SOCKET, SO_XGIGPIPE, (void*)&nOne, sizeof(int));
#endif
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BitcoinAdult Core is probably already running."), addrBind.ToString());
        else
